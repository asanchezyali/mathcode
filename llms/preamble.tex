...
%%% ALGORITHMS
\usepackage[vlined,spanish,onelanguage,linesnumbered]{algorithm2e}
...
% Light comment style for algorithms
\definecolor{algoComment}{RGB}{140, 155, 180}
\newcommand\algocomment[1]{\footnotesize\textit{\textcolor{algoComment}{#1}}}
\SetCommentSty{algocomment}

% Algorithm counter
\newcounter{algocounter}[chapter]
\renewcommand{\thealgocounter}{\thechapter.\arabic{algocounter}}

% Algorithm colors
\definecolor{algoBG}{RGB}{248, 250, 255}
\definecolor{algoBorder}{RGB}{200, 210, 235}
\definecolor{algoHeader}{RGB}{75, 85, 120}

% Elegant algorithm box
\newenvironment{algorithmbox}[2][]{%
    ...
    \begin{tcolorbox}[
        ...
        colback=algoBG,
        colframe=algoBorder,
        colbacktitle=algoHeader,
        coltitle=white,
        title={Algoritmo~\thealgocounter~--~#2},
        #1
    ]%
}{%
    \end{tcolorbox}%
...
%%% CODE QR STRIP
\definecolor{qrStripBG}{RGB}{250, 248, 245}
\definecolor{qrStripBorder}{RGB}{225, 220, 210}
\definecolor{qrStripLabel}{RGB}{120, 110, 95}

\newcommand{\codeqrstrip}[6]{%
    % #1 = Python URL, #2 = C URL, #3 = Rust URL
    % #4, #5, #6 = filenames (unused, kept for compatibility)
    \begin{center}
        \vspace{4pt}
        \begin{minipage}[t]{2.5cm}
            \centering
            \qrcode[height=2cm]{#1}\\[4pt]
            {\footnotesize\textcolor{qrStripLabel}{Python}}
        \end{minipage}%
        \hspace{1.5cm}%
        \begin{minipage}[t]{2.5cm}
            \centering
            \qrcode[height=2cm]{#2}\\[4pt]
            {\footnotesize\textcolor{qrStripLabel}{C}}
        \end{minipage}%
        \hspace{1.5cm}%
        \begin{minipage}[t]{2.5cm}
            \centering
            \qrcode[height=2cm]{#3}\\[4pt]
            {\footnotesize\textcolor{qrStripLabel}{Rust}}
        \end{minipage}
        \vspace{4pt}
    \end{center}
}

% QR sidebar: vertical QR column next to content
\newcommand{\codeqrsidebar}[3]{%
    % #1 = Python URL, #2 = C URL, #3 = Rust URL
    \begin{minipage}[t]{1.6cm}
        \vspace{0pt}
        \centering
        \qrcode[height=1.3cm]{#1}\\[2pt]
        {\tiny\textcolor{qrStripLabel}{Python}}\\[6pt]
        \qrcode[height=1.3cm]{#2}\\[2pt]
        {\tiny\textcolor{qrStripLabel}{C}}\\[6pt]
        \qrcode[height=1.3cm]{#3}\\[2pt]
        {\tiny\textcolor{qrStripLabel}{Rust}}
    \end{minipage}%
}
...
%%% MATRIX SPACING
% bmatrix with inner padding for readability
\newenvironment{bmat}{%
    \left[\;\begin{matrix}}{%
    \end{matrix}\;\right]}
