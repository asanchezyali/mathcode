...
Consideremos ahora una función $f:\RR \to \RR$. Su valor en un intervalo $[w_0, w_0 + h]$ puede ser aproximado por la secante entre los puntos $(w_0,f(w_0))$ y $(w_0 + h,f(w_0 + h))$ como una función lineal con pendiente $(f(w_0 + h) - f(w_0))/h$. En el límite cuando la variación $h$ tiende a cero alrededor de $w_0$, la secante converge a la \textbf{recta tangente} de $f$ en $w_0$, y su pendiente se define como la derivada de $f$ en $w_0$. La siguiente definición formaliza esta intuición.

\begin{definition}[Derivada]
    \label{def:derivada}
    La derivada de una función $f:\RR\to \RR$ en un punto $x_0\in \RR$ se define como 
    \[
    f'(x_0):=\lim_{h \to 0}\frac{f(x_0 + h) - f(x_0)}{h},
    \]
    siempre que este límite exista. En tal caso, diremos que $f$ es \textbf{diferenciable} en $x_0$.
\end{definition}

Si $f$ es diferenciable en cualquier $x\in \RR$, diremos que es \textbf{diferenciable en todas partes}. Si $f$ es diferenciable en un $x_0$ dado, entonces es necesariamente continua en $x_0$ como veremos en la siguiente proposición. Sin embargo continuidad no implica diferenciable como se ilustra con función de Kink.
...
La derivada nos permitir construir una aproximación lineal de una función $f$ en una vecindad de $x_0$, ya que representa la pendiente de la recta tangente de $f$ en $x_0$. Por otro lado, también nos da información sobre la \textbf{monotonicidad} de $f$ alrededor de $x_0$. 
...
\begin{align*}
    ...
    &= \frac{\sum_{k=0}^{n}\binom{n}{k}h^k x^{n-k} - x^n}{h} \\
...
\end{remark}
Para una $x\in \RR$ y dos funciones $f:\RR \to \RR$ y $g:\RR\to \RR$, la derivada de operaciones elementales entre $f$ y $g$ tales como sumas, productos o composiciones pueden ser fácilmente derivadas de la definición~\ref{def:derivada}, bajos las condiciones apropiadas de diferenciabilidad para $f$ y $g$ en $x_0$. Por ejemplo, si la derivada de $f$ y $f$ existen en $x_0$ entonces la derivada de su suma, $(af+bg)(x)$ o de su producto, $(fg)(x)$, existen y vienen dadas por:

\begin{enumerate}
    \item \textbf{Linealidad:} $(af + bg)'(x) = af'(x) + b g'(x)$, $\forall a, b \in \RR,$
    \item \textbf{Regla del producto:} $(fg)'(x) = f'(x)g(x) + f(x)g'(x)$, $\forall a, b \in \RR.$
\end{enumerate}
donde $(af + bg)(x) = af(x) + bg(x)$ y $(fg)(x) = f(x)g(x)$.
